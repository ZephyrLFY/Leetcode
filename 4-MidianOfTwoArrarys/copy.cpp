...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
       //idea: median m + n if even; median is (arr[(m+n)/2 - 1] + arr[(m+n)/2])/2
        ...
        //if k > m we can drop the smallest part of the above arr of size len so that we search k - len in the rest
        //until k == 0 return it
        if ((nums1.size() + nums2.size()) % 2 == 0) {
            return (findKth(nums1, 0, nums2, 0, (nums1.size() + nums2.size())/2)
                + findKth(nums1, 0, nums2, 0, (nums1.size() + nums2.size())/2 + 1))*0.5;
        } else {
